...
Este servicio está encargado de procesar las publicaciones. Incluye funciones para crear, obtener y borrar publicaciones.
...
- `POST /posts` - Crea una publicacion
- `GET /posts/{post_id}` - Trae una publicacion en específico
- `GET /posts` - Trae todas las publicaciones
- `DELETE /posts/{post_id}` - Borra una publicacion
...
```bash
newman run tests/api/posts_collection.json
```
> [!IMPORTANT]
...
> ```bash
> newman run tests/api/posts_collection.json --env-var "POSTS_PATH={{url}}"
> ```

## Documentación técnica

La documentación de la arquitectura del sistema puede encontrarla en TODO
