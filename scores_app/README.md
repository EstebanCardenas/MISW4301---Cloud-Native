...
Esta app está desarrollada en FastAPI que implementa una arquitectura hexagonal modificada para gestionar la entidad Score. La aplicación se ejecuta haciendo uso de docker y puede ser ejecutada directamente haciendo uso de k8s.

## Índice

- [Scores app](#scores-app)
  - [Índice](#índice)
...
kubectl apply -f .\k8s\scores-app-deployment.yaml
minikube service scores-app-service
```
...
- `POST /scores` - Crea una utilidad
- `GET /scores?postId={postId}` - Ver y filtrar utilidades por post
- `GET /scores/{id}` - Consultar una utilidad
- `DELETE /scores/{id}` - Eliminar utilidad
- `GET /scores/count` - Consultar cantidad de entidades
- `GET /scores/ping` - Consulta de salud del servicio
- `POST /scores/reset` - Restablecer base de datos
